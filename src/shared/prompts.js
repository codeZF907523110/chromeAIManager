...
/** Agent 系统提示词 —— AI 自主执行代理 */
export function buildAgentSystemPrompt(context) {
  const tools = COMMANDS.filter(
    (c) =>
      c.intent !== "unknown" &&
      c.intent !== "show_help" &&
      c.intent !== "chat" &&
  ...
  const tabs = context._truncated
    ? formatTruncated(context.tabs, 15, context.activeTab?.windowId)
    : formatFull(context.tabs, 20);

  const lessonsBlock = context.recentLessons?.length
    ? "\n## 最近经验\n" +
      context.recentLessons
        .map(function (l, i) {
          return i + 1 + ". [" + l.domain + "] " + l.error + " -> 避免重犯";
        })
        .join("\n") +
      "\n"
    : "";

  const pageBlock = context.pageStructure
    ? formatPageStructure(context.pageStructure)
    : "";
  ...
  return (
    ...
    "## 搜索操作的通用处理原则\n\n" +
    "当用户说「搜索XX」或类似意图时，按以下逻辑处理：\n\n" +
    "1. **判断当前页面是否有搜索能力**\n" +
    "   - 查看当前页面 URL 是否指向搜索引擎（通过域名判断）\n" +
    "   - 扫描页面元素，寻找搜索输入框（通常带有搜索图标、placeholder 包含「搜索」或「search」关键词、或 type/name 属性暗示搜索功能）\n" +
    "   - 如果页面有搜索能力，优先在当前页面完成搜索，不要跳转\n\n" +
    "2. **在当前页面执行搜索的方法**\n" +
    "   - 使用 dom_manipulate 工具\n" +
    "   - 先找到搜索输入框元素（通过 querySelector 查找）\n" +
    "   - 设置 value 为搜索关键词\n" +
    "   - 触发键盘事件提交（keydown、keypress、keyup 依次派发 Enter 键）\n" +
    "   - 注意：不同网站搜索框的 name 属性可能不同，需要动态查找\n\n" +
    "3. **当前页面无法搜索时**\n" +
    "   - 使用 navigate 工具打开一个通用的搜索引擎\n" +
    "   - 将用户查询词拼接到搜索 URL 参数中\n\n" +
    "4. **用户明确要求打开特定网站时**\n" +
    "   - 如「打开XX网站」「去XX」\n" +
    "   - 使用 navigate 工具直接打开目标 URL\n\n" +
    "## action 类型\n\n" +
    ...
    "11. 严禁用 navigate 代替 DOM 操作。navigate 的唯一用途是用户明确要求「打开网址」或「跳转到某页面」。搜索、填表、点击等操作必须通过 dom_manipulate 在当前页面完成。DOM 操作失败时换参数重试，连续失败 3 次后 ask 用户说明情况，绝对不能用 navigate 绕过去。\n" +
...
function formatFull(tabs, max) {
  return tabs
    .slice(0, max)
    .map(function (t) {
      return "  [" + t.id + "] " + (t.title || "(无标题)") + " | " + t.url;
    })
    .join("\n");
}

function formatTruncated(tabs, max, currentWinId) {
  var cur = tabs.filter(function (t) {
    return t.windowId === currentWinId;
  });
  var oth = tabs.filter(function (t) {
    return t.windowId !== currentWinId;
  });
  var out =
    "【当前窗口】\n" +
  ...
  if (oth.length > 0) {
    out +=
      "\n【其他窗口 · " +
      Math.min(oth.length, Math.floor(max * 0.4)) +
      " 个】\n";
    out += oth
...
function formatPageStructure(ps) {
  if (!ps || !ps.count) return "";
  var total = ps.totalCount || ps.count;
  var out =
    "\n## 当前页面 (" +
    (ps.title || ps.url) +
    ") — " +
  ...
  if (ps.elements?.length) {
    ps.elements.forEach(function (el, n) {
      var parts = [];
      if (el.text) parts.push('text="' + el.text + '"');
      if (el.attrs) {
        var keys = Object.keys(el.attrs);
  ...
  if (ps.iframes?.length) {
    out += "页内 iframe:\n";
    ps.iframes.forEach(function (f, i) {
      out +=
        "  [" +
        i +
        "] src=" +
