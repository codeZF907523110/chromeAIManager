...
class SidePanel {
  ...
  async handleSlashCommand(text) {
    ...
    if (intent === 'get_theme' && (slots.mode || slots.color)) resolvedIntent = 'set_theme'
    if (intent === 'get_font_size' && slots.size) resolvedIntent = 'set_font_size'
    if (intent === 'get_font_family' && slots.family) resolvedIntent = 'set_font_family'
  ...
  async agentLoop(userText) {
    ...
    while (stepCount < MAX_AGENT_STEPS) {
      ...
      if (result.success === false && result.code === 'NEEDS_CONFIRM') {
        this.renderConfirmation(result.detail.intent || toolName, result.detail.slots || {}, {
          ...
          onConfirm: async () => {
            const confirmResult = await this._executeCommand(toolName, {
              ...toolCall.args,
      ...
      if (consecutiveErrors >= MAX_CONSECUTIVE_FAILURES) {
        this.renderSystemMessage(`连续 ${consecutiveErrors} 步执行失败，已停止。`)
        this._cleanup()
        return
      }
    ...
    this.messageContainer.lastChild?.remove()
    this.renderAIChat('已达到最大执行步数。任务可能未完成，请继续告诉我下一步。')
    this._cleanup()
  }
    ...
    try {
      ...
      if (data.planTracker) {
        ...
        if (proceed) {
          ...
          this.renderSystemMessage('已恢复上次任务的上下文。请继续告诉我你的需求。')
        } else {
          sessionStorage.removeItem(SESSION_KEY)
        }
      }
  ...
  async _executeCommand(intent, slots) {
    ...
    try {
      let payload = slots
      if (cmd.requiresPrecompute) payload = await this.precompute(intent, slots)
  ...
  _stepSummary(result) {
    ...
    if (r.tab && r.active !== undefined)
      return r.active ? `切换到标签 *${r.tab.title || ''}*` : `更新标签 *${r.tab.title || ''}*`
    ...
    // Page
    if (r.zoomFactor !== undefined) return `缩放至 ${Math.round(r.zoomFactor * 100)}%`
  ...
  showPalette(commands) {
    if (!this._paletteEl) {
      ...
      document.addEventListener('click', (e) => {
        if (!this._paletteEl.contains(e.target) && e.target !== this.commandInput)
          this.hidePalette()
      })
    }
  ...
  navigatePalette(dir) {
    ...
    this._paletteEl
      ...
      .forEach((el, i) => el.classList.toggle('active', i === this._paletteIndex))
  }
  selectPalette() {
  ...
  async dispatchToSW(userIntent, slots) {
    ...
    let payload = slots
    if (cmd.requiresPrecompute) payload = await this.precompute(userIntent, slots)
  ...
  renderAIChat(text) {
    ...
    if (this._lastScreenshot) {
      ...
      el.appendChild(img)
      window.parent.postMessage({ type: 'COPY_SCREENSHOT', dataUrl: this._lastScreenshot }, '*')
      this._lastScreenshot = null
    }
    this.messageContainer.appendChild(el)
  ...
  renderExecutionResult(intent, response) {
    // 兼容无 intent 的调用
    if (response === undefined && intent !== undefined && typeof intent === 'object') {
      response = intent
      intent = undefined
    }
    // 处理结构化错误格式
    ...
    // intent 感知覆盖 — 优先匹配
    if (intent === 'sort_tabs' && result.moved) text = `已按域名排序 ${result.moved} 个标签`
    else if (intent === 'pin_tab') text = result.tab?.pinned ? '已固定标签' : '已取消固定'
    ...
    else if (intent === 'unmute_tabs_by_domain' && result.tab) text = '已取消静音'
      ...
      text = `为你找到 ${result.found || result.items.length} 条历史记录:\n${result.items
        .map((it) => {
          const time = it.lastVisit ? new Date(it.lastVisit).toLocaleString('zh-CN') : ''
    ...
    else if (result.uninstalled) text = `已为你卸载扩展 "${result.uninstalled}"`
    ...
    else if (result.sorted) text = `已按 ${result.order} 排序 ${result.sorted} 个标签`
    ...
    else if (result.pinned !== undefined) text = result.pinned ? '已固定标签' : '已取消固定'
    else if (result.reloaded) text = '已刷新'
    else if (result.duplicated) text = '标签已复制'
    else if (result.removed) text = `已删除 ${result.removed} 个书签`
    else if (result.storageRemoved) text = `已删除存储键 "${result.storageRemoved}"`
    ...
    else if (result.reordered) text = `已将 "${result.reordered}" 调整到第 ${result.index} 位`
    else if (result.moved && result.to) text = `已将 "${result.moved}" 移动到 ${result.to}`
      ...
      this.messageContainer.appendChild(el)
      this.scroll()
      window.parent.postMessage({ type: 'COPY_SCREENSHOT', dataUrl: result.screenshot }, '*')
      this._addMessage('system', {
        text: `[截图: ${result.tabTitle || '页面'}]`,
      })
      return
    } else if (result.zoom !== undefined) text = `缩放: ${Math.round(result.zoom * 100)}%`
    ...
    else if (typeof result.deleted === 'string') text = `已删除文件夹 "${result.deleted}"`
    else if (result.deleted)
      text = `已清理 ${result.deleted} 条${result.timeRange ? ` (${result.timeRange})` : ''}历史记录`
    else if (result.timeRange) text = `已清除${result.timeRange === 'all' ? '全部' : ''}历史记录`
    else if (result.groupsCreated !== undefined)
      text =
        result.groupsCreated > 0 ? `已创建 ${result.groupsCreated} 个分组` : '当前页面暂无需要分组'
    ...
    else if (result.folder?.title) text = `已创建书签文件夹 "${result.folder.title}"`
    ...
    else if (result.action === 'remove') text = `已删除 ${result.removed} 个元素`
    else if (result.action === 'add') text = `已添加 <${result.tag}>`
    else if (result.action === 'style') text = `已修改 ${result.changed} 个元素样式`
  ...
  renderConfirmation(intent, slots, preview, options = {}) {
    ...
    card.querySelector('.btn-confirm').addEventListener('click', async () => {
      ...
      if (options.onConfirm) {
        ...
        try {
          const response = await this.dispatchToSW(intent, { ...slots, force: true })
          card.remove()
