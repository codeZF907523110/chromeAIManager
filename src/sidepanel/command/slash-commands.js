...
/**
 * 解析斜杠命令输入
 * 返回 { intent, slots } 或 null（如果不是斜杠命令）
 * 返回 { intent, slots, error } 如果无法匹配
 */
export function matchSlashCommand(input) {
  ...
  // 1. 精确匹配
  let cmd = SLASH_COMMANDS.find((c) => c.slash === cmdName);
  // 2. 别名匹配
  ...
  if (!cmd)
    cmd = SLASH_COMMANDS.find(
      (c) =>
        c.slash.startsWith(cmdName) ||
        c.aliases?.some((a) => a.startsWith(cmdName)),
    );
  ...
  if (cmd.intent === "show_help") {
    return { intent: "show_help", slots: { commands: SLASH_COMMANDS } };
  }
  ...
  if (cmd.hasArg && args) {
    switch (cmd.intent) {
      ...
      case "search_history":
        ...
        break;
      case "delete_history":
        slots.timeRange = args;
        break;
      case "group_tabs": {
        const [groupName, ...patternParts] = parts.slice(1);
        slots.groupName = groupName || "新分组";
        if (patternParts.length > 0) slots.pattern = patternParts.join(" ");
        break;
      }
      case "sort_tabs":
        slots.order = args;
      ...
      case "discard_tabs":
        ...
        else slots.domain = args;
        break;
      case "unmute_tabs_by_domain":
        slots.domain = args;
        break;
      case "zoom_tab":
        slots.direction = args;
        break;
      case "screenshot":
        slots.query = args || undefined;
        break;
      case "rename_group":
        slots.name = args;
        break;
      case "get_theme":
        if (args) {
          if (args.startsWith("color ")) slots.color = args.slice(6);
          else slots.mode = args;
        }
        break;
      case "get_font_size":
        if (args) slots.size = args;
        break;
      case "get_font_family":
        if (args) slots.family = args;
        break;
      case "new_window":
        slots.url = args || undefined;
        break;
      case "get_cookies":
      case "clear_cookies":
        slots.domain = args;
      ...
      case "set_site_permission": {
        ...
        slots.domain = permParts[0] || "";
        slots.setting = permParts[1] || "";
        slots.value = permParts[2] || "allow";
        break;
      }
      case "storage_get":
      case "storage_remove":
      ...
      case "storage_set": {
        ...
        if (idx > 0) {
          slots.key = args.slice(0, idx);
          slots.value = args.slice(idx + 1);
        } else {
          slots.key = args;
          slots.value = "";
        }
